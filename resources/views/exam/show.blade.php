    ...
    <div class="py-10">
        ...
        <div class="flex gap-3 px-5">
            ...
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg p-7 w-1/4">
                <h1 class="text-slate-900 font-extrabold text-2xl sm:text-xl lg:text-2xl mb-3">Pengaturan Ujian</h1>
                <hr>
                <div class="mb-3">
                    <form action="{{ route("exam.update", $exam->uuid) }}" method="post">
                        @method("PUT")
                        <select class="w-full" name="available">
                            <option value="true" @if($exam->available == "true") selected @endif>Tersedia</option>
                            <option value="false" @if($exam->available == "false") selected @endif>Belum Tersedia</option>
                        </select>
                        @csrf
                        <div class="my-2">
